...
const makeAttendanceUnique = () => {
	rl.question('please enter mongodb url ', (answer) => {
        ...
        Attendance.find().sort({ _id: -1 }).limit(200).exec().then(async data => {
            data.forEach(async item => {
                let uniqueCustomers = [];
                let uniqueAbsentees = [];
                item.customers.forEach(customer => {
                    if(!uniqueCustomers.some(uniqueCustomer => customer.equals(uniqueCustomer))){
                        uniqueCustomers.push(customer)
                    }
                })
                item.absentees.forEach(customer => {
                    if(!uniqueAbsentees.some(uniqueCustomer => customer.equals(uniqueCustomer))){
