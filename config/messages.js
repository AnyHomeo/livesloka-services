module.exports = {
  SUCCESSFUL_SUBSCRIPTION: (amount,currency,subject,isResubscribe,startDate,timeWithTimeZone,dueDate) => `
Namaskaram,

Thank you for your payment of ${amount} ${currency.toUpperCase()}.  You have successfully registered for ${subject} regular classes. ${isResubscribe ? "" : `Your first class is on ${startDate} at  ${timeWithTimeZone}.`} We request you to attend all the classes as per the schedule given in https://mylivesloka.com. Your next payment due date is on ${dueDate}. We once again welcome you to Live Sloka Family!
...
ADMIN_PAYMENT_SUCCESSFUL: (amount,currency,subject,customer,isResubscribe,startDate,dueDate) => `
Dear Admin,

You have successfully received a payment of ${amount} ${currency.toUpperCase()}  for ${subject} regular classes from Mr/Mrs ${customer} ${isResubscribe ? "" : `from ${startDate}`}.  Their next payment due date is on ${dueDate}. 

Regards 
Your CRM
`,

SUCCESSFUL_DEMO_SCHEDULE: (customer,phone,subject,teacher,dateWithTime) => `
Dear Admin,
...
ADMIN_PAYMENT_UNSUCCESSFUL: (customer,dateTime,amount,currency,subject,dueDate,phone) => `
Dear Admin,
