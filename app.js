...
io.on("connection", (socket) => {
	...
	socket.on("messageFromBot", async ({roomID, message, name}, callback) => {
		try {
			...
			socket.to(roomID).emit("messageToRoomFromBot", {role: 1, message, name})
		} catch (error) {
			if (error) return callback(error)
		}
	...
	// 	callback()
	// })
	socket.on("messageFromAdmin", async ({roomID, message, isSuperAdmin, name}, callback) => {
	...
	socket.on("agent-to-agent-assign", ({agentID, roomID, assigneID, user}, callback) => {
		try {
			socket.broadcast.emit("agent-to-agent-assign", {agentID, roomID, assigneID, user})
		} catch (error) {
			if (error) return callback(error)
		}
		callback()
