...
module.exports = {
				...
				try {
					...
					if (data) {
						...
						if (req.body.firstName && req.body.lastName) {
							...
							user.username = 'Livesloka User ' + Math.floor(Math.random() * 1000000 + 1);
						}
						if (req.body.email) {
						...
						newUserdata
							...
							.then((newUser) => {
								return res.status(200).send({
									status: 'OK',
									message: 'Customer data inserted and Login Credentials created Successfully',
									result: val,
								});
							})
							.catch((err) => {
				...
				.then((data) => {
					data = data.map((customer) => {
						if (customer.subjectId) {
							return {
								...customer,
								subject: subjects.filter((subject) => subject.id == customer.subjectId)[0],
							};
						} else {
							return customer;
						}
				...
				customers.map(async (customer) => {
					...
					if (customer.paidTill) {
						...
						isJoinButtonDisabled =
							(dateArr[2] > dateToday[2] && dateArr[1] > dateToday[1] && dateArr[0] > dateToday[0]) ||
					...
					if (actualSchedule) {
						...
						let selectedZoneUTCArray = allZones.filter((zone) => zone.abbr === timeZone.timeZoneName)[0]
							.utc;
	...
	insertDataFromWix: async (req, res) => {
		try {
			...
			data['field:comp-kk7je66v'] =
				typeof data['field:comp-kk7je66v'] === 'string' ? data['field:comp-kk7je66v'].toUpperCase() : 'IST';
			let timeZoneSelected = await TimeZoneModel.findOne({
				timeZoneName: data['field:comp-kk7je66v'],
			}).lean();
			let selectedSubjectNames = data['field:comp-kk7je66r1'].split(',').map((name) => name.trim());
			...
			let user = {
				...
				gender:
					typeof data['field:comp-kk926n0p'] === 'string' ? data['field:comp-kk926n0p'].toLowerCase() : '',
	...
	getCustomersAllDataByUserIdSettings: async (req, res) => {
		const isValidSettings = (filters) => Object.keys(filters).some((filterKey) => filters[filterKey].length);
	...
	getUserTimeZone: async (req, res) => {
		try {
			...
			const customerTZId = await CustomerModel.findById(customerId).select('timeZoneId');
			if (customerTZId) {
				const timeZone = await TimeZoneModel.findOne({
					id: customerTZId.timeZoneId,
				}).lean();

				let selectedZoneUTCArray = allZones.filter((zone) => zone.abbr === timeZone.timeZoneName)[0].utc;
	...
	updateLastTimeJoined: async (req, res) => {
		try {
			...
			let allUsersWithThatEmail = await CustomerModel.find({ email }).select('_id email firstName').lean();
			allUsersWithThatEmail = allUsersWithThatEmail.map((user) => user._id);
			let schedule = await ScheduleModel.findById(scheduleId).select('students').lean();
			let selectedCustomer = schedule.students.filter((student) =>
				allUsersWithThatEmail.some((user) => user.equals(student))
			)[0];
			let response = await CustomerModel.updateOne({ _id: selectedCustomer }, { lastTimeJoined: new Date() });
	...
	getStatistics: async (req, res) => {
		try {
			...
			let allSchedules = await SchedulerModel.find({
				...
				.select('slots students meetingLink teacher className subject lastTimeJoinedClass demo')
				.populate('subject', 'subjectName')
				.populate('students', 'firstName lastTimeJoined lastName numberOfClassesBought email whatsAppnumber')
				.lean();
