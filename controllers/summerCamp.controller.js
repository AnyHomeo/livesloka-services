...
const getPaymentLink = (amount, title, currency, customerId, res) => {
    ...
    transactions: [
      {
        ...
        description: 'Payment for Summer camp ' + title || 'of Livesloka',
      },
    ],
  };
  paypal.payment.create(payment_json, (error, payment) => {
    if (error) {
      ...
      let url = payment.links.filter((link) => link.rel === 'approval_url')[0]
        .href;
      return res.json({
...
exports.getSummerCampSchedules = async (req, res) => {
  try {
    ...
    let allTeachers = await TeacherModel.find()
      .select('id TeacherName teacherImageLink TeacherDesc')
      .lean();
...
exports.registerCustomer = async (req, res) => {
  try {
    ...
    let alreadyExists = await CustomerModel.findOne({
      email,
      tempScheduleId: scheduleId,
    });
    ...
    req.body.firstName =
      req.body.firstName + ' ' + schedule.subject.subjectName;
    if (!alreadyExists) {
      ...
      return getPaymentLink(
        schedule.summerCampAmount,
        schedule.summerCampTitle,
        'USD',
        newCustomer._id,
        res
      );
      ...
      if (isPaymentDone) {
        ...
        let currency = await CurrencyModel.findOne({
          id: alreadyExists.proposedCurrencyId,
        });
        if (currency) {
          currency = currency.currencyName;
        }
        return getPaymentLink(
          alreadyExists.proposedAmount,
          schedule.summerCampTitle,
          currency,
          alreadyExists._id,
          res
  ...
  try {
    ...
    paypal.payment.execute(
      ...
      async (error, payment) => {
        if (error) {
          console.log(error);
          return res.status(400).send('Error Validating Payment!');
        } else {
          ...
          let newUpdate = new ClassHistoryModel({
            ...
            nextValue: customer.numberOfClassesBought
              ? customer.numberOfClassesBought +
                schedule.summerCampClassNumberOfDays
              : schedule.summerCampClassNumberOfDays,
            comment: 'Successful Payment!',
            customerId: customerId,
          });
          await newUpdate.save();
          customer.numberOfClassesBought = customer.numberOfClassesBought
            ? customer.numberOfClassesBought +
              schedule.summerCampClassNumberOfDays
            : schedule.summerCampClassNumberOfDays;
          await customer.save();
          ...
          newPayment
            ...
            .then(async (data) => {
              await client.messages.create({
                body: `Live Sloka: Payment Successful!, ${
                  data._id ? 'PaymentId: ' + data._id : ''
                }. Your Slot for ${
                  schedule.summerCampTitle
                } is booked.For any queries please reachout to info@livesloka.com`,
...
exports.onSummerCampFailurePayment = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send('Internal Server Error, Try again after Sometime or Contact Admin');
  }
};
