...
exports.validateSlot = (req, res, next) => {
	if (!req.body.slot) {
		...
		} else if (!(!isNaN(arr[1].split(':')[0]) && parseInt(arr[1].split(':')[0]) <= 12)) {
			return res.status(400).json({ stage: 3, error: 'Invalid Entry' });
		} else if (!(arr[2].split(':')[1].startsWith('30') || arr[2].split(':')[1].startsWith('00'))) {
			return res.status(400).json({ stage: 4, error: 'Invalid Entry' });
		} else {
			let { slot } = req.body;
			req.body.slot =
				slot.split('-')[0].trim() + '-' + slot.split('-')[1].trim() + '-' + slot.split('-')[2].trim();
			next();
		}
	}
};
...
exports.addSlot = (req, res) => {
	...
	TeacherModel.findOne({
		...
		if (data.availableSlots && data.scheduledSlots) {
			if (!data.availableSlots.includes(req.body.slot) && !data.scheduledSlots.includes(req.body.slot)) {
	...
	CustomerModel.find({ classStatusId: '113975223750050' })
		.then((students) => {
			TeacherModel.find({})
				...
				.catch((err) => {
					return res.status(400).json({ message: 'error in retrieving data ', err });
				});
		})
		.catch((err) => {
			console.log(err);
...
exports.getOccupancyDashboardData = async (req, res) => {
	try {
		...
		let allTeachers = await TeacherModel.find().select('id TeacherName availableSlots scheduledSlots category');
		...
		allCategories.forEach((category) => {
			...
			allTeachers.forEach((teacher) => {
				if (teacher.category === category.id) {
					const { TeacherName, scheduledSlots, availableSlots, _id, id } = teacher;
...
exports.getAllDaysSlots = async (req, res) => {
	const { id } = req.params;
	try {
		let availableSlotsData = await TeacherModel.findOne({ id }).select('availableSlots -_id');
...
exports.GetSalaries = async (req, res) => {
	try {
		...
		let teacherAttends = await Attendence.find({
			$and: [{ teacherId: { $in: allTeacherIds } }, { date: { $regex: dat, $options: 'm' } }],
		}).populate('scheduleId');
		let finalObj = [];
		allTeachers.forEach((teacher) => {
			let objj = {
...
exports.joinClass = async (req, res) => {
	try {
		...
		let schedule = await SchedulerModel.findById(scheduleId).select('meetingLink wherebyHostUrl');
		if (schedule) {
...
exports.getTeacherLeavesAndSchedules = async (req, res) => {
	try {
		const { id } = req.params;
		let allSchedulesOfThatTeacher = await SchedulerModel.find({ teacher:id, isDeleted: { $ne: true } })
			.select('students slots className demo isClassTemperarilyCancelled message')
    ...
    let entireDayLeavesDays = leavesOfTeacher.filter(leave => leave.entireDay).map(leave => momentTZ(leave.date).tz('Asia/Kolkata').format('dddd').toUpperCase())
      let sortedSchedules = {}
    allSchedulesOfThatTeacher.forEach((schedule) => {
			Object.keys(schedule.slots).forEach((slot) => {
        let isOnLeave = leavesOfTeacher.findIndex(leave => leave.scheduleId && schedule._id.equals(leave.scheduleId)) !== -1;
        schedule.slots[slot].forEach(hour => {
          sortedSchedules[hour] = {
            ...schedule,
            slots:undefined,
            isOnLeave: isOnLeave || entireDayLeavesDays.includes(hour.split("-")[0])
          }
        })
      });
		});
