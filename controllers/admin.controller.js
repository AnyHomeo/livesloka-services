...
module.exports = {
	authentication(req, res) {
		try {
			admin.findOne(
				{
					$or: [{ userId: req.body.userId.toLowerCase() }, { userId: req.body.userId }],
				},
				async (err, user) => {
					console.log(err);
					if (!user) {
...
module.exports.getComments = (req, res) => {
	Comment.find({ customerId: req.params.id })
		.then((result) => {
			res.status('200').send({ message: 'comment got successfully', status: 'ok', result });
		})
		.catch((err) => {
...
module.exports.updatecomment = (req, res) => {
	Comment.updateOne({ _id: req.body._id }, req.body)
		.then((result) => {
			res.status('200').send({ message: 'Updated  successfully', status: 'ok', result });
		})
		.catch((err) => {
...
module.exports.deletecomment = (req, res) => {
	Comment.deleteOne({ _id: req.body._id })
		.then((result) => {
			res.status('200').send({ message: ' Deleted  successfully', status: 'ok', result });
		})
		.catch((err) => {
			res.status('400').send({ message: 'something went wrong !!', err });
		});
...
module.exports.getinvoices = (req, res) => {
	if (req.body.start == '') {
		let d = new Date();
		req.body.start = d.getFullYear().toString() + '-' + (d.getMonth() + 1).toString() + '-01';
		req.body.end = d.getFullYear().toString() + '-' + (d.getMonth() + 1).toString() + '-30';
	}
	...
	InvoiceModel.find({ invoiceDate: { $gte: startDate, $lte: endDate } })
		.then((val) => {
			let parse = (d) => {
				...
				return givenDate.getDay() + '-' + givenDate.getMonth() + '-' + givenDate.getFullYear();
			};
...
module.exports.validateOtpAndResetPassword = async (req, res) => {
    ...
    console.log(userId,otp)
		...
		await admin.save();
		return res.json({ message: "Password Reset Successful!, Use 'Livesloka' as Temporary password" });
	} catch (error) {
		console.log(error);
		return res.status(500).json({ error: 'error in password reset,Try again later' });
	}
