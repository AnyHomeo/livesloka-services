...
exports.addSchedule = async (req, res) => {
	...
	try {
		...
		if (classname) {
			...
			className = `${selectedSubject.subjectName} ${selectedTeacher.TeacherName} ${startDate} ${
				demo ? 'Demo' : ''
			}`;
		}
	} catch (error) {
	...
	try {
		...
		try {
			...
			if (className) {
				...
				req.body.className = `${selectedSubject.subjectName} ${selectedTeacher.TeacherName} ${startDate} ${
					demo ? 'Demo' : ''
				}`;
			}
		} catch (error) {
		...
		let oldScheduleSlots = Object.keys(oldSchedule.slots).map((day) => oldSchedule.slots[day].sort());
		...
		if (isNewMeetingLinkNeeded) {
			ZoomAccountModel.findById(meetingAccount, async (err, data) => {
				...
				let { monday, tuesday, wednesday, thursday, friday, saturday, sunday } = slots;
				let availableZoomAccount = await ZoomAccountModel.findOne({
					timeSlots: {
						$nin: [...monday, ...tuesday, ...wednesday, ...thursday, ...friday, ...saturday, ...sunday],
					},
				});
...
exports.deleteScheduleById = async (req, res) => {
	...
	try {
		...
		let slotsOfSchedule = monday
			...
			.concat(saturday)
			.concat(sunday);
		teacherOfSchedule.availableSlots = teacherOfSchedule.availableSlots.concat(slotsOfSchedule);
		teacherOfSchedule.availableSlots = [...new Set(teacherOfSchedule.availableSlots)];
...
exports.getAllScheduleswithZoomAccountSorted = async (req, res) => {
	try {
		...
		const allZoomAccounts = await ZoomAccountModel.find().select('color ZoomAccountName').lean();
...
exports.dangerousScheduleUpdate = async (req, res) => {
	...
	try {
		...
		let updatedSchedule = await SchedulerModel.updateOne({ _id: scheduleId }, { ...req.body });
		return res.json({
			...
			if (isMeetingLinkChangeNeeded || oldSchedule.isZoomMeeting) {
				if (oldSchedule.isZoomMeeting) {
					...
					let deletedMeeting = await fetch(
						'https://api.whereby.dev/v1/meetings/' + oldSchedule.wherebyMeetingId,
						{
							method: 'delete',
							headers: {
								Authorization: `Bearer ${process.env.WHEREBY_API_KEY}`,
				...
				let meetingLinkData = await fetch('https://api.whereby.dev/v1/meetings', {
					method: 'post',
					headers: {
						Authorization: `Bearer ${process.env.WHEREBY_API_KEY}`,
						'Content-type': 'application/json',
